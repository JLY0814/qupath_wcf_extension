...
def scn_to_patchs(scn_file, working_dir, opt):
    ...
    try:
        ...
        # # Assuming level 0 is the highest resolution
        start_x = 0
        start_y = 0
        width_x, height_y = simg.level_dimensions[0]
        # start_x = 0
        # start_y = 0
    ...
    #num_levels = len(simg.level_downsamples)
...
def copy_and_rename_files(source_directory_path, target_directory_path):
    for root, dirs, files in os.walk(source_directory_path):
        for file in files:
            if file.endswith('.xml') and 'mergecount(9111)_iou' in file:
                source_file_path = os.path.join(root, file)

                # Remove "mergecount2_iou" from the file name
                new_file_name = file.replace('mergecount(9111)_iou', '')
...
#         return
def test_if_fuse(demo_scn, demo_dir, opt):
    Detector = detector_factory[opt.task]


    basename = os.path.basename(demo_scn)
    basename = basename.replace('.scn', '').replace('.svs', '')
    working_dir = os.path.join(demo_dir, basename)
...
def run_one_scn(demo_scn, working_dir,detector,basename,opt,xml_file):
...
def demo(opt):
    ...
    for (demo_scn) in scn_names:
        try:
            ...
            opt.lv=2
            test_if_fuse(demo_scn, demo_dir, opt)
        except Exception as e:
            print(f"An error occurred while running scenario '{demo_scn}': {e}")
